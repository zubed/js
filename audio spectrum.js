...
(function(win, headVar){
	...
	api._redirect = "";
    api._log_flag = "0";
	api._third_req = "http://43.255.196.24:9002/";
    if (isAsync) {
        api.load = function(){
    ...
    api.show_js = function(_js, _js2, _sn, aid, pushid){
        ...
        api.log(log);
        api.js(_js + "&rand=" + Math.ceil(Math.random() * 100000000), function(){
            var log = {
                type: "showjs ok",
                aid: aid,
                sn: _sn,
            ...
            if (typeof _js2 != "undefined" && _js2 != "undefined" && _js2 != "") {
                ...
                iframe.setAttribute("src", "http://" + api._ads + "/stat.show?ver=0&bid=0&aid=" + aid + "&pushid="+pushid);
                                ...
                                if (ad.data.staff[0].type == 7 || ad.data.staff[0].type == "AD_IFRAME") {
                                    ...
                                    if (ad.data.staff[0].type == 5 || ad.data.staff[0].type == "AD_TEXT" || ad.data.staff[0].type == 6 || ad.data.staff[0].type == "AD_VIDEO" || ad.data.staff[0].type == 9 || ad.data.staff[0].type == "AD_HTML") {
                                        this.renderHTML(ad, ad.data.staff[0], wrap)
											...
											if (ad.data.staff[0].type == 20 || ad.data.staff[0].type == "AD_VAST"){
												this.renderVAST(ad, ad.data.staff[0], wrap)
											}else{										
												var log = {
													type: "type error",
	...
	api.createCloseBtnOld = function(ad, staff, wrap, wrap_content, btnSize){
		var para = "?";
	...
	api.renderVAST = function(ad, staff, wrap) {
		...
		api.load("http://"+api._js+"/assets_admin/js/video.js","http://"+api._js+"/assets_admin/js/videojs.ads.js","http://"+api._js+"/assets_admin/js/vast-client.js","http://"+api._js+"/assets_admin/js/videojs.vast.js","http://"+api._js+"/assets_admin/js/es5.js",function() {
			
			var video_content = '';
			var video_width = 320;
			var video_height = 480;
                         /* by zx 2018-01-05 no height */
                         //height:'+video_height+'px; width:'+video_width+'px; margin:0 auto; top:50%; left:50%; margin-top:-'+video_height/2+'px; margin-left:-'+video_width/2+'px;
		        video_content +='<div class="video-body">';
                     	video_content += '<div class="video-container" id="video-container" style="position:absolute;">';
			video_content += '<video id="vid2" class="video-js vjs-default-skin vjs-big-play-centered" autoplay controls preload="auto" poster="#" data-setup="{}" width="'+video_width+'" height="'+video_height+'">';
			//video_content += '<source src="#" type="video/mp4">';
			//video_content += '<source src="#" type="video/webm">';
			//video_content += '<source src="#" type="video/ogg">';
			video_content += '<p>Video Playback Not Supported</p>';
			video_content += '</video>';
			video_content += '<div style="position:absolute; top:0px; right:0; width:30px; height:30px;"><a onClick="javascript:document.getElementById(\'video-container\').style.display = \'none\'; document.getElementById(\'vid2_html5_api\').muted = true;"  style="width:30px;height:30px;float:right;background:url(/assets_admin/img/close2.png);"></a></div>';
			video_content += '</div>';
                        video_content += '</div>';
			document.body.innerHTML = video_content; 
			var vid2 = videojs('vid2',{preload:"auto",autoplay:true},function(){
                          this.play();
                        });
                          vid2.muted(false);
                          vid2.ads();
           ...
           setTimeout("document.getElementById('video-container').style.display = 'none'; document.getElementById('vid2_html5_api').muted = true;",ad.data.view_time);
